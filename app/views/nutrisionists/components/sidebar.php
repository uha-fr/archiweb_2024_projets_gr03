...
<?php
 $nutritionistDetails = $data['nutritionistDetails'];
?>
...
<div class="flex flex-wrap items-center cursor-pointer">
       <img src='<?php echo IMG_LOAD . '' . $nutritionistDetails["image"]?>' class="w-10 h-10 rounded-full object-cover	  border-2 border-white" />
        <div class="ml-4">
          <p class="text-sm text-white"><?php echo (strtoupper($nutritionistDetails['lastname']) ." ". ucfirst($nutritionistDetails['firstname'])) ?></p>
          <p class="text-xs text-gray-300 mt-1"><?php echo $nutritionistDetails['email'] ?></p>
        </div>
      </div>
      ...
      <li>
        ...
        <li>
          <a href="<?php echo BASE_URL."dashboard/clients" ?>" class="text-white text-sm flex items-center hover:bg-[#755f5a] rounded px-4 py-2 transition-all">
